...
def Info_Rank():
    conexao = conectar()
    cursor = conexao.cursor(dictionary=True)
    consulta = "SELECT Rank, Nome, Pontos, Placar FROM ClassificacaoGeral"
    cursor.execute(consulta)
...
def Info_Adm(seq):
    conexao = conectar()
    cursor = conexao.cursor(dictionary=True)
    consulta = "SELECT  Adm FROM Usuario WHERE Seq = %s and Adm like 'S'"
    cursor.execute(consulta, (seq))
    resultado = cursor.fetchone()
    cursor.close()
    conexao.close()
...
def Info_Cabecalho(seq_usuario):
    ...
    consulta = """
        SELECT 
        IFNULL(c.Nome, 'Desconhecido') AS Nome,
        6 AS Seq,
        IFNULL(c.Data_Atu, CURRENT_DATE()) AS Data_Atu,
        IFNULL(c.Pontos, 0) AS Pontos,
        IFNULL(c.Placar, 0) AS Placar,
...
def ativar_rodada_01():
    conexao = conectar()
    cursor = conexao.cursor()
    atualizax = "UPDATE Jogos SET StatusRodada = 'Ativo' WHERE Rodada LIKE '15'" 
...
def verificar_rodada_ativa():
    conexao = conectar()
    cursor = conexao.cursor(dictionary=True)
    consulta = "SELECT Rodada,StatusRodada FROM  Jogos WHERE   StatusRodada = 'Ativo' GROUP BY Rodada,StatusRodada"
    cursor.execute(consulta, ())
    resultado = cursor.fetchone()
    cursor.close()
    conexao.close()
...
def atualizar_online(rodada_ativa):
    ...
    #Inserir Rodada
    comando6 = f' UPDATE   Jogos SET Pontos = 2 WHERE Resultado = Palpite AND StatusRodada LIKE "Ativo" AND Pontos < 3   AND Palpite <> "Pendente" ;'
